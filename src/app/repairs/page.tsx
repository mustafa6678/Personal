...
export default function RepairsPage() {
  return (
    <main className="pt-20 bg-white">
        ...
        <Container>
          <div className="max-w-4xl px-4">
            <motion.p
              ...
              className="text-violet-400 font-black uppercase tracking-[0.4em] text-[8px] md:text-[10px] mb-6"
            >
            ...
            >
              Expert <br />
              <span className="text-violet-500">Diagnostics.</span>
            </motion.h1>
            <p className="text-base md:text-xl text-slate-400 font-bold italic mb-10 md:mb-12 leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 px-4">
            {services.map((service, index) => (
              <motion.div
                ...
                className="group p-8 md:p-10 bg-slate-50 border border-slate-100 rounded-[2rem] md:rounded-[3rem] hover:bg-white hover:shadow-2xl hover:border-violet-100 transition-all duration-500"
              >
                <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg group-hover:bg-violet-600 transition-colors mb-8 md:mb-10">
                  <service.icon className="w-6 h-6 md:w-8 md:h-8 text-violet-600 group-hover:text-white" />
                </div>
                
                <h3 className="text-2xl md:text-3xl font-black text-slate-950 uppercase italic tracking-tighter mb-4 leading-tight">{service.title}</h3>
                <p className="text-sm md:text-base text-slate-500 font-bold italic mb-8 leading-relaxed line-clamp-3">{service.desc}</p>
                
                <div className="space-y-3 mb-10">
                  {service.features.map((feature) => (
                    <div key={feature} className="flex items-center gap-3">
                      <CheckCircle2 className="w-4 h-4 text-violet-600" />
                      <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-400">{feature}</span>
                    </div>
                  ))}
                </div>

                <div className="flex items-center justify-between pt-6 border-t border-slate-100">
                  <span className="text-xl md:text-2xl font-black text-slate-950 italic">{service.price}</span>
                  <Link href="/contact">
                    <button className="px-6 py-2.5 md:px-8 md:py-3 bg-slate-950 text-white rounded-xl text-[8px] md:text-[10px] font-black uppercase tracking-widest hover:bg-violet-600 transition-all">
                      Book Now
                    </button>
                  </Link>
                </div>
      ...
      <section className="py-16 md:py-24 bg-violet-600 text-white overflow-hidden">
        <Container>
      ...
      <section className="py-24 md:py-40 bg-white">
        <Container>
          <div className="max-w-4xl mx-auto text-center px-4">
            <h2 className="text-4xl md:text-6xl lg:text-8xl font-black tracking-tighter text-slate-950 uppercase italic leading-[0.9] mb-10 md:mb-12">
              Start Your <br />
              <span className="text-slate-200">Restoration.</span>
            </h2>
            <Link href="/contact" className="block w-full sm:w-auto">
              <button className="w-full sm:w-auto px-12 py-5 md:py-6 bg-slate-950 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-xs hover:bg-violet-600 transition-all shadow-2xl flex items-center justify-center gap-4 mx-auto group">
                Request Free Quote
